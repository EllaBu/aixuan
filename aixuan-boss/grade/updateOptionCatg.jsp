...
<body>
	...
	<div id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li><span>修改分项</span><span class="divider"></span></li>
		</ul>
	</div>
...
<!--#breadcrumbs-->
	<div id="page-content" class="clearfix">
		<div class="page-header position-relative crumbs-nav">
			<h1><i class="icon-plus"></i><span>修改 </span>&nbsp;&nbsp;<span id="series_name"></span></h1>
					...
					<div class="span12">
						<div class="form-inline form-inline-detail bord1dd form-add1 mar-t-15">
							...
							<div class="from-group mar-t-15">
								...
								<input id="oc_ratio" type="text" placeholder="" >
								<b>%</b>
							</div>
							<div class="from-group mar-t-15">
							...
							<div class="from-group">
								<button id="update" class="user-add-save" type="reset">保存</button>
							</div>
						</div>
...
<script type="text/javascript">
	...
	var oc_id = getURLparam("oc_id");//分项id
	...
	function getOneOptionCatg() {
		...
		var options = {
			url : "/grade/getOneOptionCatg.xco",
			data : xco,
			success :function(data){
				if(data.getCode()==0){
					var obj=data.getData().getXCOListValue("list");
					obj = obj[0];
					$("#oc_name").val(obj.get("oc_name"));
					if(oc_score_type==1){
						$("#oc_type_duty").val(obj.get("oc_type"));
					}
					if(oc_score_type==2){
						$("#oc_type_disease").val(obj.get("oc_type"));
					}					
					$("#oc_content_type").val(obj.get("oc_content_type"));
					var oc_ratio = obj.get("oc_ratio");
					oc_ratio = oc_ratio/100;
					$("#oc_ratio").val(oc_ratio);
					$("#oc_sort").val(obj.get("oc_sort"));
				}else{
	...
	$("#update").click(function(){
		...
		xco.setIntegerValue("oc_id", oc_id);
		if(oc_score_type ==1){
			xco.setStringValue("sys_op_log","编辑责任分分项-："+oc_name);
		}else{
			xco.setStringValue("sys_op_log","编辑病种分分项-："+oc_name);
		}			
		var options = {
			url : "/grade/updateGOptionCatg.xco",
			data : xco,
			success : function(data){
				if(data.getCode()!=0){
					axError(data.getMessage());
