		...
		<div id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li><a href="../organization/orgList.jsp">公司管理</a><span
					class="divider"><i class="icon-angle-right"></i> </span></li>
				<li>修改公司<span class="divider"></span></li>
			</ul>
		</div>
...
<script type="text/javascript">
	...
	function getOrg(){
		var xco = new XCO();
		xco.setLongValue("org_no",orgNo);
		var options = {
			url : "/org/getOrg.xco",
			data : xco,
			success : getOrgCallBack
		};
		
		$.doXcoRequest(options);
	}
	
	//通过org_no获取公司基本资料成功回调
	function getOrgCallBack(data){
		if(data.getCode()!=0){
			axError(data.getMessage());
		}else{
			data = data.getData();
			$("#fullName").val(data.getStringValue("full_name"));
			$("#name").val(data.getStringValue("name"));
			$("#imgUrl").val(data.getStringValue("img_url"));
			$("#content").val(data.getStringValue("content"));
			if(data.getStringValue("content")){
				$("#text-count2").text(data.getStringValue("content").length);
			}
			var n_url = fileurl+data.getStringValue("img_url");
	...
	$("#updateOrg").click(function(){
		...
		//console.log(xco);
		xco.setStringValue("sys_op_log","编辑公司-编辑："+name);
		var options = {
			url : "/org/updateOrg.xco",
			data : xco,
			success : function(data){
				if(data.getCode()!=0){
					axError(data.getMessage());
