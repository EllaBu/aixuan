...
<body>
	...
	<div id="breadcrumbs">
		<ul class="breadcrumb">
			...
			<li><span>修改产品规则</span><span class="divider"></span>
    			</li>
		</ul>
	</div>
					...
					<div class="span12">
						...
						<input type="hidden" id="up_id">
							...
							<div class="from-group mar-t-15">
								...
								<input type="text" class="form-control mar-t-15" id="org_no" placeholder="">
							</div>
							<div class="from-group mar-t-15">
								<label><span style="color:red;">*</span>保险公司</label>
								<input type="text" class="form-control mar-t-15" id="org_name" placeholder="">
							</div>
							<div class="from-group mar-t-15">
								<label>产品分类</label>
								...
								<!-- <select class="form-control w150" id="sub_series_no">
									<option value="0">请选择</option>
								</select>  -->
							</div>
							<div class="from-group mar-t-15" >
...
<script type="text/javascript">
	...
	var up_id = getURLparam("up_id");
	dogetProduct();
	function dogetProduct(){
		var xco=new XCO();
		xco.setLongValue("up_id", up_id);
		var options = {
			url : "/securityMultiple/getProductById.xco",
			data : xco,
			success : function(data){
				if(data.getCode()==0){
					var obj=data.getData();
					$("#up_id").val(obj.get("up_id"));
					$("#ch_id").val(obj.get("ch_id"));
					$("#rule_id").val(obj.get("rule_id"));
					$("#org_no").val(obj.get("org_no"));
					$("#org_name").val(obj.get("org_name"));
					$("#series_no").val(obj.get("series_no"));
					/* $("#sub_series_no").val(obj.get("sub_series_no")); */
					$("#up_ext_pno").val(obj.get("up_ext_pno"));
					$("#up_ext_pname").val(obj.get("up_ext_pname"));
					$("#up_intl_pno").val(obj.get("up_intl_pno"));
					$("#up_intl_pname").val(obj.get("up_intl_pname"));
					$("#up_desc").val(obj.get("up_desc"));
					$("#up_type").val(obj.get("up_type"));
					$("input[name='up_state']").each(function(i,el){
...
/* 		if(series_no==0){
		...
		xco.setLongValue("up_id",$("#up_id").val());
		xco.setLongValue("ch_id",$("#ch_id").val());
		xco.setLongValue("rule_id",rule_id);
		xco.setLongValue("org_no",org_no);
		xco.setStringValue("org_name",$("#org_name").val());
		xco.setLongValue("series_no",$("#series_no").val());
		...
		xco.setStringValue("up_ext_pname",$("#up_ext_pname").val());
		xco.setStringValue("up_intl_pno",$("#up_intl_pno").val());
		xco.setStringValue("up_intl_pname",$("#up_intl_pname").val());
		xco.setStringValue("up_desc",$("#up_desc").val());
		xco.setIntegerValue("up_type",$("#up_type").val());
		xco.setIntegerValue("up_state",$("input[name='up_state']:checked").val());
		var options = {
			url : "/security/updateProduct.xco",
			data : xco,
			success : function(data){
				if(data.getCode()!=0){
